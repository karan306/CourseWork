...
void getfield(char* line,int x,int y,char c[]){
...
void csv_data(int x, int y,char D[],char E[]){
...
uint f(uint a,uint b,uint c,uint d){
    uint z=0;
    z+=d;
    z+=(c*8);
    z+=(b*64);
...
uint bytee(uint a,uint b,uint c,uint d){
    uint z=0;
    z+=d;
    z+=(c*(1<<8));
    z+=(b*(1<<16));
...
void getZ(uint buf[],int* a,int* b){
    uint x=bytee(buf[3],buf[2],buf[1],buf[0]);
	
    x=decrypt(609721191,x);
    *b=x%16;
    x/=16;*a=x%16;
}

void total(uint buf[],uint ret[]){
    char D[8],E[8];
    for(int i=0;i<8;i++){
        D[i]='z';
        E[i]='0';
    ...
    for(int i=0;i<8;i++){
        ...
        if(D[i]=='z'){
            ...
            ans[i]=f(1,D[i]-'0',i,E[i]-'0');
        }
    }
    uint a=bytee(ans[0],ans[1],ans[2],ans[3]),b=bytee(ans[4],ans[5],ans[6],ans[7]);
    a=encrypt(609721191,a);b=encrypt(609721191,b);
    ret[7]=b%(1<<8);
    b/=(1<<8);
    ret[6]=b%(1<<8);
    b/=(1<<8);
...
bool check_ack(uint8 b[]){
    uint x=bytee(b[3],b[2],b[1],b[0]),y=1219412015;
    x=decrypt(609721191,x);
    if(x==y) return true;
    return false;
}
...
void modify(uint8 buf[],int x,int y){
	uint z=bytee(buf[3],buf[2],buf[1],buf[0]);
	z=decrypt(609721191,z);
	int nh=z%8,di,o,e;z/=8;di=z%8;z/=8;o=z%2;z/=2;e=z%2;
	char c[35]="sed -i '/2,2,4.*/d' track_data.csv";c[35]='\0';
	//x=2;y=2;di=4;nh=6;o=3;
	c[9]= '0'+x;c[11]='0'+y;c[13]='0'+di;
	int status = system(c);
	char d[36]="echo '1,1,1,1,1'  >> track_data.csv";d[36]='\0';
	d[6]='0'+x;d[8]='0'+y;d[10]='0'+di;d[12]=o+'0';d[14]=nh+'0';
	status = system(d);
	...
	if ( shellOpt->count ) {
		...
		if ( isCommCapable ) {
		   ...
		   uint X[4];
			...
			if ( isRunning ) {
				do {
					    uint a = encrypt(609721191,90165850);
					...
					do{
						fStatus = flReadChannel(handle,channel,4,buffer1,&error) ; // coords
						for(int i=0;i<4;i++)
						   {
							printf("%u ",buffer1[i]);
							X[i]=buffer1[i];
				  		   }
						fStatus = flWriteChannel(handle,channel+1,4,buffer1,&error); // coords
						printf("\nFPGAlink 3\n");
						fStatus = flReadChannel(handle,channel,4,buffer5,&error) ; // ack
						printf("\nFPGAlink 4\n");
						if(!check_ack(buffer5)){
							sleep(5);
							if(channel == 126)
								channel = 0;
							else
					...
					fStatus = flWriteChannel(handle,channel+1,4,buffer3,&error) ; // ack

					printf("\nFPGAlink 5\n");
					total(X,buffer2);
					   for(int i=0;i<4;i++)
					   {
						buffer2a[i]=buffer2[i];
			  		   }
					   for(int i=0;i<4;i++)
					   {
						buffer2b[i]=buffer2[i+4];
			  		   }
					fStatus = flWriteChannel(handle,channel+1,4,buffer2a,&error); // look up 4 bytes ???
					printf("\nFPGAlink 6\n");
					clock_t begin = clock();
					float tim;
					do{
					fStatus = flReadChannel(handle,channel,4,buffer5,&error) ; // ack
					tim = (clock()-begin)/CLOCKS_PER_SEC;
					}while(!check_ack(buffer5) && tim < 257);
					
					if(tim>256) continue;
											
					printf("\nFPGAlink 7\n");
					fStatus = flWriteChannel(handle,channel+1,4,buffer2b,&error); // look up 4 bytes ???
					printf("\nFPGAlink 8\n");
					...
					fStatus = flReadChannelAsyncSubmit (handle,channel,4,NULL,&error) ; // coords
					fStatus = flReadChannelAsyncAwait(handle, &recvData, &actualLength, &actualLength, &error);
					for(int i=0;i<4;i++)
						   {
							printf("%u ",recvData[i]);
