		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for l in range(len(self.legalLabels)):
					if self.legalLabels[l]==trainingLabels[i]:
						ind=l
				for l in range(ind):
					if self.weights[frozenset([l,ind])]*trainingData[i]<=0:
						self.weights[frozenset([l,ind])]=self.weights[frozenset([l,ind])]+trainingData[i]
				for l in range(ind+1,len(self.legalLabels)):
					if self.weights[frozenset([l,ind])]*trainingData[i]>0:
						self.weights[frozenset([l,ind])]=self.weights[frozenset([l,ind])]-trainingData[i]
